...
def test_cart_strings():
    ...
    assert (
        Strings.Cart.CHECKOUT_ERROR_EMPTY == "Cannot place order. Is your cart empty?"
    )
    ...
    # Test cart methods
    assert (
        Strings.Cart.item_line("Apple", 2, 1.50, 3.00) == "- Apple (2 x $1.50) = $3.00"
    )
...
def test_general_strings():
    ...
    assert (
        Strings.General.welcome_new_user("Alice")
        == "Welcome, Alice! \n\nI am the PalsPantry Bot. I can help you browse our inventory and place orders."
    )
    assert (
        Strings.General.welcome_returning_user("Bob")
...
def test_error_strings():
    ...
    assert (
        Strings.Error.UNKNOWN_COMMAND
        == "Sorry, I didn't understand that. Please use /start to begin or /shop to browse the menu."
    )
...
def test_product_strings():
    ...
    assert (
        Strings.Product.ASK_DESCRIPTION.format(name="Test Product")
        == "Name set to 'Test Product'.\n\nNow, please enter a description."
    )
    assert (
        Strings.Product.ASK_QUANTITY.format(price=19.99)
        == "Price set to $19.99.\n\nHow many units are available? (e.g., 10)"
    )
    assert (
        Strings.Product.ASK_CATEGORY.format(qty=5)
