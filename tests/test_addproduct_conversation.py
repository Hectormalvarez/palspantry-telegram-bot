...
@pytest.mark.asyncio
async def test_received_product_category_valid_text_input_shows_confirmation( # Renamed for clarity
    mocker,
    mock_update_message: Update,
    mock_telegram_context: ContextTypes.DEFAULT_TYPE
):
    """
    Test that after receiving a valid category, a confirmation message with all details
    and an inline keyboard is shown, and state transitions to PRODUCT_CONFIRMATION.
    """
    # Setup: Simulate previous data collection
    product_details = {
        'name': 'Test Widget',
        'description': 'A truly amazing test widget for all your widgeting needs.',
        'price': 19.99,
        'quantity': 10
    }
    mock_telegram_context.user_data = {'new_product': product_details.copy()} # Use .copy()
    ...
    # Action
    next_state = await bot_main.received_product_category(mock_update_message, mock_telegram_context)
    ...
    # Get the arguments of the call to reply_text
    # call_args is a tuple, call_args[0] are positional args, call_args[1] are keyword args
    # For older mock versions, it might be call_args.args and call_args.kwargs
    # With pytest-mock, call_args is a unittest.mock.call object
    args, kwargs = mock_update_message.message.reply_text.call_args

    # 1. Assert the summary message text
    expected_summary = (
        "Okay, let's confirm the details for your new product:\n\n"
        f"Name: {product_details['name']}\n"
        f"Description: {product_details['description']}\n"
        f"Price: ${product_details['price']:.2f}\n" # Assuming USD formatting for price
        f"Quantity: {product_details['quantity']}\n"
        f"Category: {category_name}\n\n"
        "Is everything correct?"
    )
    assert args[0] == expected_summary # args[0] is the first positional argument (the text)

    # 2. Assert the InlineKeyboardMarkup
    assert 'reply_markup' in kwargs
    reply_markup = kwargs['reply_markup']
    assert isinstance(reply_markup, InlineKeyboardMarkup)
    
    assert len(reply_markup.inline_keyboard) == 1 # Assuming one row of buttons
    buttons = reply_markup.inline_keyboard[0]
    assert len(buttons) == 3
    
    assert buttons[0].text == "✅ Confirm & Save"
    assert buttons[0].callback_data == "product_confirm_save" # Define callback data
    
    assert buttons[1].text == "✏️ Edit" # Simplified Edit for now
    assert buttons[1].callback_data == "product_confirm_edit"
    
    assert buttons[2].text == "❌ Cancel"
    assert buttons[2].callback_data == "product_confirm_cancel"
    
    # Ensure previous data is preserved for the confirmation step
    assert 'name' in mock_telegram_context.user_data['new_product']
    assert 'quantity' in mock_telegram_context.user_data['new_product']
